    ...
    for (privileged_data, unprivileged_data) in valid_pairs {
        for privileged_execute in [false, true] {
            for unprivileged_execute in [false, true] {
                ...
                let round_trip = <VmsaAttributeCodec as AttributeCodec<
                    Vmsa64,
                    NonSecureEl1Stage1,
                    Granule4KiB,
                    _,
...
pub fn d128_stage2_base_matrix() -> TestResult {
    ...
    let mut failures = 0;
    let base_raw = <VmsaAttributeCodec as AttributeCodec<
        Vmsa128,
        NonSecureEl2Stage2<Stage2Permissions>,
        Granule4KiB,
        _,
...
pub fn d128_stage2_overlay_matrix() -> TestResult {
    ...
    let mut failures = 0;
    let template = <VmsaAttributeCodec as AttributeCodec<
        Vmsa128,
        NonSecureEl2Stage2<Stage2Permissions>,
        Granule4KiB,
        _,
...
pub fn invalid_d128_alias() -> TestResult {
    ...
    let resolved = <VmsaAttributeCodec as AttributeCodec<
        Vmsa128,
        NonSecureEl1Stage1,
        Granule4KiB,
        _,
